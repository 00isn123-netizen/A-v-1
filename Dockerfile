...
# 1. Base packages (added boost, opencl-headers, ocl-icd, pocl for KNLMeansCL)
RUN pacman -Syu --noconfirm && \
    ...
    boost opencl-headers ocl-icd pocl && \
...
# 4. VapourSynth Plugins (Using Official Upstream URLs)
...
RUN git clone --depth 1 https://github.com/Khanattila/KNLMeansCL.git /tmp/knlm && \
    cd /tmp/knlm && \
    meson setup build --buildtype=release && \
    ninja -C build && ninja -C build install && \
    rm -rf /tmp/knlm

RUN git clone --depth 1 https://github.com/Irrational-Encoding-Wizardry/descale.git /tmp/descale && \
    cd /tmp/descale && \
    meson setup build --buildtype=release && \
    ninja -C build && ninja -C build install && \
    rm -rf /tmp/descale

RUN git clone --depth 1 https://github.com/dubhater/vapoursynth-nnedi3.git /tmp/nnedi3 && \
    cd /tmp/nnedi3 && \
    meson setup build --buildtype=release && \
    ninja -C build && ninja -C build install && \
    rm -rf /tmp/nnedi3
